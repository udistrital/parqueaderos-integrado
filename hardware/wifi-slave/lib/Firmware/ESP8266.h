...
public:
   ...
   *
   * @warning parameter baud depends on the AT firmware. 9600 is an common
   * value.
   */
   ...
   *
   * Actually, this method will send command "AT" to ESP8266 and waiting for
   * "OK".
   *
   * @retval true - alive.
   * @retval false - dead.
   ...
   * @note This method will occupy a lot of memeory(hundreds of Bytes to a
   * couple of KBytes).
   *  Do not call this method unless you must and ensure that your board has
   * enough memery left.
   */
   ...
   */
  bool setSoftAPParam(String ssid, String pwd, uint8_t chl = 7,
                      uint8_t ecn = 4);
   ...
   *
   * In multiple connection mode, a couple of TCP and UDP communication can be
   * builded.
   ...
   *
   * In single connection mode, only one TCP or UDP communication can be
   * builded.
   *
   * @retval true - success.
   * @retval false - failure.
   ...
   *
   * @param timeout - the duration for timeout by second(0 ~ 28800,
   * default:180).
   * @retval true - success.
   * @retval false - failure.
   */
   ...
   *
   * After started, user should call method: getIPStatus to know the status of
   * TCP connections.
   * The methods of receiving data can be called for user's any purpose. After
   * communication,
   * release the TCP connection is needed by calling method: releaseTCP with
   * mux_id.
   *
   ...
   * @see uint32_t recv(uint8_t *coming_mux_id, uint8_t *buffer, uint32_t len,
   * uint32_t timeout);
   ...
   *
   * After return, coming_mux_id store the id of TCP or UDP from which data
   * coming.
  ...
  int httpGet(char *request, char *ip, uint32_t port);
...
private:
  ...
  void rx_empty(void);
  /*
   * Recvive data from uart. Return all received data if target found or
   * timeout.
   */
  String recvString(String target, uint32_t timeout = 1000);

  /*
   * Recvive data from uart. Return all received data if one of target1 and
   * target2 found or timeout.
   */
  String recvString(String target1, String target2, uint32_t timeout = 1000);

  /*
   * Recvive data from uart. Return all received data if one of target1, target2
   * and target3 found or timeout.
   */
  String recvString(String target1, String target2, String target3,
                    uint32_t timeout = 1000);

  /*
   * Recvive data from uart and search first target. Return true if target
   * found, false for timeout.
   */
  bool recvFind(String target, uint32_t timeout = 1000);

  /*
   * Recvive data from uart and search first target and cut out the substring
   * between begin and end(excluding begin and end self).
   ...
   * @param data_len - the length of data actually received(maybe more than
   * buffer_size, the remained data will be abandoned).
   * @param timeout - the duration waitting data comming.
   * @param coming_mux_id - in single connection mode, should be NULL and not
   * NULL in multiple.
   */
