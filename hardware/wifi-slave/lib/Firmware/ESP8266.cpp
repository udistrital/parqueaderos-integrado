...
bool ESP8266::autoSetBaud(uint32_t baudRateSet) {
  rx_empty();
  long time0 = millis();
  long baudRateArray[] = {9600, 19200, 57600,
                          115200}; // These are the optional default baudrates
  ...
  for (int j = 0; j < attempts;
       j++) { // attempt to connect to esp over each baudrate
    for (int i = 0; i < sizeof(baudRateArray);
         i++) // check for current esp baudrate
    {
      ...
      while (m_puart->available()) {
        ...
        if (inData.indexOf("OK") !=
            -1) { // if OK received, this is the current baudrate of the ESP
          baudFlag = 1;
          delay(15);
          break;
        }
...
// when using software serial BaudRate should be lower than 115200. 9600 works
// reliably
bool ESP8266::init(const String &ssid, const String &pwd,
                   uint32_t baudRateSet) {
...
bool ESP8266::kick(void) { return eAT(); }
...
bool ESP8266::joinAP(String ssid, String pwd) { return sATCWJAP(ssid, pwd); }

bool ESP8266::leaveAP(void) { return eATCWQAP(); }
...
String ESP8266::getLocalIP(void) {
  ...
  while (m_puart->available() > 0) {
    ...
    if (inData.indexOf("IP") != -1) {
      delay(100);
      return ("IP: " +
              inData.substring(inData.indexOf("IP") + 4, inData.length() - 2));
    }
  }
  return "Couldn't get IP adress";
}

bool ESP8266::enableMUX(void) { return sATCIPMUX(1); }
...
bool ESP8266::unregisterUDP(void) { return eATCIPCLOSESingle(); }
...
bool ESP8266::startServer(uint32_t port) { return startTCPServer(port); }

bool ESP8266::stopServer(void) { return stopTCPServer(); }
...
// when using software serial due to buffer issues read incoming string char by
// char
#ifdef ESP8266_USE_SOFTWARE_SERIAL
      char c = m_puart->read();
      buffer[i++] = c;
